...
See [what's new](/CHANGELOG.md) in the latest release.

This documents the crate as it currently exists in `src/`. There is
currently no `viewset` (CRUD/ORM) module in this crate — an earlier version
of actixutils included one, but it was extracted into a separate crate (see
the CHANGELOG entry for `0.6.0-a`). If you're looking for that
functionality, look for a standalone `viewset` crate instead.
...
Plus a standalone `pubkey` module for serving an RSA public key (see below).
...
key-value backend, and all of them share the same trait: `locals::Store<K,
V>`. You implement it once per backend (in-memory, Redis, a database
table, ...) and reuse it for:
...
process). This is a separate, more general trait from `locals::IdempotencyStore`
and `middleware::cache::CacheStore`, which are TTL-aware trait families used only
by `Idempotency`.
...
backed directly by the general-purpose `locals::Store<Uuid, T>` described above.
