    ...
    async fn list(
        ...
        if let Some(sort) = &query.sort {
            let clauses: Vec<String> = PaginationParams::parse_sort(sort)
                .into_iter()
                .filter(|(field, _)| {
                    <Self::Entity as Entity>::SORTABLE.contains(&field.as_str())
                })
                .map(|(field, dir)| {
                    format!(
                        "{field} {}",
...
fn fields_from_dto<E: Entity>(dto: &(impl serde::Serialize + ?Sized)) -> Vec<(&'static str, SqlValue)> {
...
fn push_filters<E: Entity>(qb: &mut QueryBuilder<Postgres>, query: &QueryParams, has_where: &mut bool) {
