...
use actix_web::{ 
    Error, FromRequest, HttpMessage, HttpRequest, body::MessageBody, 
...
impl<T> Session<T>{ 
    ...
    pub async fn write(&self) -> tokio::sync::RwLockWriteGuard<'_, T>{ // <-- &self not &mut self
...
impl<S> SessionMiddleware<S> { 
    pub fn new(store: Arc<S>) -> Self { 
        Self { store, cookie_name: "session".into(), } 
    } 
            ...
            let (session_id, session, new_session) = match req.cookie(&cookie_name) { 
                Some(cookie) => { 
                    ...
                    let id = match Uuid::parse_str(cookie.value()){ 
                        Ok(id) => id, 
                        Err(_) => { new_session = true; Uuid::new_v4() } 
                    }; 
